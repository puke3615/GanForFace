...
class DCGAN_Expand():
    ...
    def build_generator(self):
        ...
        convs = []
        n_fc = 1
        n_item = self.latent_dim // n_fc
        n_fc_unit = 2 * 64 * 64 // n_fc
        for i in range(n_fc):
            start = i * n_item
            end = (i + 1) * n_item
            x_item = Lambda(lambda x: x[:, start: end], name='%d_%d' % (start, end))(noise)
            x_item = Dense(n_fc_unit, activation='relu')(x_item)
            x_item = Reshape((64, 64, n_fc_unit // (64 * 64)))(x_item)
            convs.append(x_item)

        x = convs[0] if len(convs) == 1 else Concatenate()(convs)

        x = Conv2DTranspose(512, kernel_size=3, padding="same", activation='relu')(x)
        x = BatchNormalization(momentum=0.8)(x)
        x = Dropout(0.25)(x)

        x = Conv2DTranspose(256, kernel_size=3, padding="same", activation='relu')(x)
        x = BatchNormalization(momentum=0.8)(x)
        x = Dropout(0.25)(x)

        x = Conv2DTranspose(128, kernel_size=3, padding="same", activation='relu')(x)
        x = BatchNormalization(momentum=0.8)(x)
        x = Dropout(0.25)(x)
