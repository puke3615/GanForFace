...
from data_loader import load_data, ANIME_PATH
...
class DCGAN_Expand():
    ...
    def build_generator(self):
        ...
        x = Dense(8 * self.img_cols * self.img_rows, activation='relu')(noise)

        x = Reshape((self.img_cols, self.img_rows, 8))(x)
        x = BatchNormalization(momentum=0.8)(x)

        x = Conv2DTranspose(256, kernel_size=5, strides=1, padding="same", activation='relu')(x)
        x = BatchNormalization(momentum=0.8)(x)
    ...
    def train(self, epochs, batch_size=128, save_interval=50):
        ...
        generator = load_data(batch_size, self.img_rows, self.img_cols, imgaug=False, path=ANIME_PATH)
